...
Service-based implementation of Interactive Brokers connection management
Migrated from src/core/ib_connection.py as part of architecture consolidation
"""
...
class IBConnectionService(BaseService):
    """
    Service for managing IB API connection
    ...
    def __init__(self):
        if not self._service_initialized:
            super().__init__("IBConnectionService")
            self.ib = IB()
    ...
    def initialize(self) -> bool:
        ...
        try:
            logger.info("IBConnectionService initialized")
            return True
        except Exception as e:
            logger.error(f"Failed to initialize IBConnectionService: {str(e)}")
            return False
    ...
    def cleanup(self):
        ...
        if self.is_connected():
            self.ib.disconnect()
        logger.info("IBConnectionService cleaned up")
    ...
    async def connect(self, trading_mode: str = None) -> bool:
        ...
        try:
            ...
            try:
                await asyncio.wait_for(self.ib.reqAccountSummaryAsync(), timeout=5.0)
            except asyncio.TimeoutError:
                logger.warning("Account summary request timed out - continuing anyway")
            
            # Request positions with timeout to avoid blocking
            try:
                await asyncio.wait_for(self.ib.reqPositionsAsync(), timeout=5.0)
            except asyncio.TimeoutError:
                logger.warning("Positions request timed out - continuing anyway")
...
# Legacy compatibility class - CRITICAL for backward compatibility
class IBConnectionManager:
    """
    Legacy compatibility wrapper for IBConnectionService
    Maintains 100% API compatibility while delegating to the service
    """
    ...
    def __init__(self):
        if not self._initialized:
            # Create and wrap the service
            self._service = IBConnectionService()
            self._service.initialize()
            
            # Mirror ALL attributes for complete compatibility
            self.ib = self._service.ib
            self.connection_state = self._service.connection_state
            self.last_connection_time = self._service.last_connection_time
            self.reconnect_attempts = self._service.reconnect_attempts
            self.max_reconnect_attempts = self._service.max_reconnect_attempts
            self.reconnect_delay = self._service.reconnect_delay
            self.trading_mode = self._service.trading_mode
            self.current_port = self._service.current_port
            self._callbacks = self._service._callbacks
            self._accounts = self._service._accounts
            self._active_account = self._service._active_account
            self._account_values = self._service._account_values
            self._account_summaries = self._service._account_summaries
            self._positions = self._service._positions
            self._orders = self._service._orders
            self._initialized = True
    
    # Delegate ALL methods to maintain perfect compatibility
    async def connect(self, trading_mode: str = None) -> bool:
        result = await self._service.connect(trading_mode)
    ...
    async def reconnect(self):
        result = await self._service.reconnect()
        self._sync_attributes()
        return result
    
    def subscribe_to_event(self, event_type: str, callback: Callable):
        self._service.subscribe_to_event(event_type, callback)
    
    def unsubscribe_from_event(self, event_type: str, callback: Callable):
        self._service.unsubscribe_from_event(event_type, callback)
    
    def get_accounts(self) -> List[str]:
        return self._service.get_accounts()
    
    def set_active_account(self, account: str) -> bool:
        result = self._service.set_active_account(account)
        self._sync_attributes()
        return result
    
    def get_active_account(self) -> Optional[str]:
        return self._service.get_active_account()
    
    def get_account_values(self, account: Optional[str] = None) -> Dict[str, Any]:
        return self._service.get_account_values(account)
    
    def get_all_account_values(self) -> Dict[str, Dict[str, Any]]:
        return self._service.get_all_account_values()
    ...
    def get_all_positions(self) -> Dict[str, List[Any]]:
        return self._service.get_all_positions()
    
    def get_orders(self) -> List[Trade]:
        return self._service.get_orders()
    
    def get_ib_client(self) -> IB:
        return self._service.get_ib_client()
    
    def set_trading_mode(self, mode: str):
        self._service.set_trading_mode(mode)
        self._sync_attributes()
    
    def get_trading_mode(self) -> str:
        return self._service.get_trading_mode()
    
    def get_current_port(self) -> int:
        return self._service.get_current_port()
    
    def is_paper_trading(self) -> bool:
        return self._service.is_paper_trading()
    
    def is_live_trading(self) -> bool:
        return self._service.is_live_trading()
    
    async def switch_trading_mode(self, new_mode: str) -> bool:
        result = await self._service.switch_trading_mode(new_mode)
        self._sync_attributes()
        return result
    
    def _sync_attributes(self):
        """Sync all attributes from service to maintain compatibility"""
        self.ib = self._service.ib
        self.connection_state = self._service.connection_state
        self.last_connection_time = self._service.last_connection_time
        self.reconnect_attempts = self._service.reconnect_attempts
...
# Create singleton instance - CRITICAL for backward compatibility
ib_connection_manager = IBConnectionManager()


# Export ConnectionState for compatibility
__all__ = ['ConnectionState', 'IBConnectionService', 'IBConnectionManager', 'ib_connection_manager']