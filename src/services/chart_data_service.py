"""
Chart Data Service
Service-based implementation of chart data management
Migrated from src/core/chart_data_manager.py as part of architecture consolidation
"""

import asyncio
from typing import List, Dict, Optional, Any
from datetime import datetime
from ib_async import BarData, Stock
...
class ChartDataService(BaseService):
    """
    Service for managing chart data fetching and formatting
    ...
    def __init__(self):
        ...
        super().__init__("ChartDataService")
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info("ChartDataService initialized successfully")
            return True
        except Exception as e:
            logger.error(f"Failed to initialize ChartDataService: {str(e)}")
            return False
            
    def cleanup(self):
        """Cleanup service resources"""
        self.cached_data.clear()
        logger.info("ChartDataService cleaned up")
...
class ChartDataManager:
    """
    Legacy compatibility wrapper for ChartDataService
    Maintains the original API while delegating to the service
    """
    
    # Copy class attributes for compatibility
    CHART_TIMEFRAMES = ChartDataService.CHART_TIMEFRAMES
    CHART_DURATIONS = ChartDataService.CHART_DURATIONS
    ...
    def __init__(self):
        ...
        self.current_symbol = self._service.current_symbol
        self.current_timeframe = self._service.current_timeframe
        self.cached_data = self._service.cached_data
        
    def get_chart_data(self, symbol: str, timeframe: str = '5m', max_bars: int = 500) -> List[Dict[str, Any]]:
        """Delegate to service"""
        return self._service.get_chart_data(symbol, timeframe, max_bars)
        
    def get_available_timeframes(self) -> List[str]:
        """Delegate to service"""
        return self._service.get_available_timeframes()
    ...
    def set_current_symbol(self, symbol: str):
        """Delegate to service"""
        self._service.set_current_symbol(symbol)
        self.current_symbol = self._service.current_symbol
        
    def set_current_timeframe(self, timeframe: str):
        """Delegate to service"""
        self._service.set_current_timeframe(timeframe)
        self.current_timeframe = self._service.current_timeframe
        
    # Private method delegation
    def _get_historical_bars_sync(self, symbol: str, duration: str, bar_size: str, max_bars: int) -> Optional[List[BarData]]:
        """Delegate to service"""
        return self._service._get_historical_bars_sync(symbol, duration, bar_size, max_bars)
        
    def _convert_to_chart_format(self, bars: List[BarData]) -> List[Dict[str, Any]]:
        """Delegate to service"""
        return self._service._convert_to_chart_format(bars)
...
def get_chart_data_service() -> ChartDataService:
    """Get the chart data service instance"""
